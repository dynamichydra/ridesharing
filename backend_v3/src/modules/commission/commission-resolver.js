...
export async function resolveDeterministicCommissionRule({
  ...
  // ── Tier 1: City + Vehicle Type + Plan Tier ──
  if (cityId && vehicleTypeId && planTierId) {
    ...
    if (match) return formatResolvedRule(match, 'city_vehicle_tier');
  }

  // ── Tier 2: City + Vehicle Type (Exact Local) ──
  ...
  // ── Tier 3: City Default (cityId set, vehicleTypeId is NULL) ──
  if (cityId) {
    const [match] = await db
      .select()
      .from(commissionRules)
  ...
  // ── Tier 5: Country Default (both cityId & vehicleTypeId are null) ──
  if (countryId) {
    const [match] = await db
      .select()
      .from(commissionRules)
  ...
  // ── Tier 6: Global Default (cityId, countryId, and vehicleTypeId all null) ──
