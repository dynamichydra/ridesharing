...
export async function recalculateTripFare({
  ...
  // 3. Apply fare-rule multiplier (locked from original trip snapshot) then baseline zone multiplier
  //    This mirrors the new engine pipeline: rule → baseline → dynamic surge
  const ruleMultiplier = surgeInfo.ruleMultiplier ? parseFloat(surgeInfo.ruleMultiplier) : 1.0;
  const baselineMultiplier = surgeInfo.baselineMultiplier ? parseFloat(surgeInfo.baselineMultiplier) : 1.0;
  const ruleAdjustedMinor = Math.round(meteredSubtotalMinor * ruleMultiplier);
  const surgeableBaseMinor = Math.round(ruleAdjustedMinor * baselineMultiplier);

  // 4. Apply locked dynamic surge multiplier on top of rule-adjusted fare
  const dynamicSurgeMultiplier = surgeInfo.dynamicSurgeMultiplier ? parseFloat(surgeInfo.dynamicSurgeMultiplier) : 1.0;
  let surgeAmountMinor = 0;
  if (dynamicSurgeMultiplier > 1.0) {
    surgeAmountMinor = Math.round(surgeableBaseMinor * (dynamicSurgeMultiplier - 1.0));
  }

  // 5. Minimum fare floor (on rule+baseline+surge adjusted fare)
  const subtotalBeforeMin = surgeableBaseMinor + surgeAmountMinor;
  const minFareApplied = subtotalBeforeMin < minFareMinor;
  const clampedMeteredMinor = Math.max(subtotalBeforeMin, minFareMinor);

  // 6. Non-surgeable fees & extras
