...
export async function promoRoutes(app) {
  ...
  // GET /api/v1/promos/usages
  app.get('/usages', { preHandler: [authenticateAdmin] }, async (request, reply) => {
    const { page, limit, offset } = parsePagination(request.query);
    const filters = {
      promoId: request.query.promoId,
      userId: request.query.userId,
      rideId: request.query.rideId,
      status: request.query.status,
    };
    const { rows, pagination } = await promoService.listPromoUsages(filters, page, limit, offset);
    return sendList(reply, rows, pagination);
  });
