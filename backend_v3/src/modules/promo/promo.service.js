  ...
  return db.transaction(async (tx) => {
    ...
    if (inserted) {
      await tx.update(promos)
        .set({ usedCount: sql`${promos.usedCount} + 1`, updatedAt: now })
...
export async function listPromoUsages(filters = {}, page = 1, limit = 20, offset = 0) {
  const conditions = [];
  if (filters.promoId) conditions.push(eq(promoUsages.promoId, filters.promoId));
  if (filters.userId) conditions.push(eq(promoUsages.userId, filters.userId));
  if (filters.rideId) conditions.push(eq(promoUsages.rideId, filters.rideId));
  if (filters.status) conditions.push(eq(promoUsages.status, filters.status));
  const where = conditions.length ? and(...conditions) : undefined;

  const [{ total }] = await db.select({ total: count() }).from(promoUsages).where(where);
  const rows = await db
    .select({
      usage: promoUsages,
      promo: promos,
    ...
    .from(promoUsages)
    .leftJoin(promos, eq(promoUsages.promoId, promos.id))
    .leftJoin(users, eq(promoUsages.userId, users.id))
    .where(where)
    .orderBy(desc(promoUsages.usedAt))
    .limit(limit)
    .offset(offset);
