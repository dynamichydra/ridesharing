...
export function recomputeActualFare({ pricing, breakdown, estimatedFareMinor, actualDistanceKm, actualDurationMin, roundingIncrementMinor }) {
  // pricing is a snapshot of a pricing_versions row — field names match the DB columns
  const baseFareMinor = pricing.baseFareMinor ?? 0;
  const distanceFareMinor = Math.round(actualDistanceKm * (pricing.perKmRateMinor ?? 0));
  const timeFareMinor = Math.round(actualDurationMin * (pricing.perMinRateMinor ?? 0));
