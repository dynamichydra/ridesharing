...
export function ZoneHexModal({
  ...
  // Compute boundary coordinates and H3 hex cells
  const { boundaryCoords, hexCells, resolution } = useMemo(() => {
    if (!zone) return { boundaryCoords: [], hexCells: [], resolution: 9 };

    const res = zone.resolution ?? 9;
    const parsedCoords = extractPolygonPoints(zone.polygon);

    let cells: string[] = [];
    if (Array.isArray(zone.hexCells) && zone.hexCells.length > 0) {
      cells = zone.hexCells;
    } else if (typeof zone.hexCells === "string") {
      try {
        const parsed = JSON.parse(zone.hexCells);
    ...
    // Dynamic generation if not pre-populated
    if (cells.length === 0 && parsedCoords.length >= 3) {
      ...
      if (rings) {
        try {
          ...
          console.warn("h3-js polygonToCells error:", err);
        }
      }
    }
        ...
        hexCells.forEach((cellId) => {
          try {
            ...
            const hexPoly = new window.google.maps.Polygon({
              ...
              fillOpacity: 0.32, // Clear 32% tint
              map: map,
              zIndex: 15,
              clickable: true,
            });
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2.5">
              ...
              <DialogTitle className="text-base font-bold text-foreground flex items-center gap-2">
                {zone?.name || "Operational Zone"}
                {zone?.type && (
                  <Badge variant="outline" className="capitalize text-[11px] font-medium font-mono">
                    {zone.type}
                  </Badge>
                )}
                {zone?.isActive ? (
                  <Badge className="bg-emerald-500/15 text-emerald-600 border-none text-[11px]">
                    Active
                  </Badge>
                ) : (
                  <Badge className="bg-muted text-muted-foreground border-none text-[11px]">
                    Disabled
                  </Badge>
                )}
              </DialogTitle>
        ...
        <div className="relative flex-1 min-h-0 w-full bg-muted/30 overflow-hidden">
          {!mapLoaded && !loadError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-xs z-20 gap-3">
              ...
              <p className="text-sm font-medium text-muted-foreground">
                Loading Google Maps & Zone Boundary...
              </p>
            </div>
          )}
          ...
          {!hasValidBoundary && mapLoaded && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-300 rounded-md px-3 py-1.5 text-xs flex items-center gap-2 shadow-md">
              <AlertCircle className="h-4 w-4 shrink-0" />
              <span>No valid polygon boundary coordinates found for this zone.</span>
            </div>
          )}
