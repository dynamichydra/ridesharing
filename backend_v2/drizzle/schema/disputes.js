...
// force either into a shared enum. Never auto-resolved on its own, but an admin can now push a
// decision *to* the processor via acceptDispute/contestDispute in dispute.service.js — the
// resulting processor status still only lands here once accept/contest returns (or a later
// webhook redelivery confirms it), same status-driven resolution path either way.
export const disputes = pgTable('disputes', {
  ...
  evidence:            jsonb('evidence'), // last evidence payload submitted via contestDispute, for audit visibility
  evidenceSubmittedAt: timestamp('evidence_submitted_at'),
  evidenceSubmittedBy: uuid('evidence_submitted_by').references(() => admins.id),
