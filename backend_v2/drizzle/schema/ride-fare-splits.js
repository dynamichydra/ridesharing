...
import { fareSplitStatusEnum, fareSplitPaymentStatusEnum } from './enums.js';
...
export const rideFareSplits = pgTable('ride_fare_splits', {
  ...
  status: fareSplitStatusEnum('status').default('pending').notNull(), // pending | accepted | declined | cancelled | expired
  splitAmountMinor: integer('split_amount_minor'), // calculated at completion or request time
  paymentStatus: fareSplitPaymentStatusEnum('payment_status').default('pending').notNull(), // pending | paid | failed
