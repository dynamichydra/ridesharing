...
export function ServiceAreaMapDrawer({
  ...
  const insertStarterPolygon = useCallback(() => {
    ...
    // 0.05 deg is approx 5km. We scale it depending on zoom level so it always fits on screen nicely.
    ...
    placePolygon(starterPts, false); // Don't snap bounds since we made it in the center
    
    // Automatically trigger sync to update parent state
  ...
  }, [mapLoaded, containerEl, polygonJson, placePolygon, renderContextOverlays]); // eslint-disable-line react-hooks/exhaustive-deps
