...
class StartPhoneAuthentication extends AuthEvent {
  ...
  StartPhoneAuthentication({required this.phone, required this.deviceId});
}
...
class VerifyOtpCode extends AuthEvent {
  ...
  final String otp;
  final String deviceId;
  VerifyOtpCode({required this.phone, required this.otp, required this.deviceId});
}
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc({required this.authRepository}) : super(AuthInitial()) {
    ...
    on<StartPhoneAuthentication>((event, emit) async {
      emit(AuthLoading());
      try {
        final success = await authRepository.startPhoneAuth(event.phone, event.deviceId);
        if (success) {
          emit(AuthOtpSent());
        } else {
          emit(AuthError(message: 'Failed to request OTP. Try again.'));
        }
      } catch (e) {
    ...
    on<VerifyOtpCode>((event, emit) async {
      emit(AuthLoading());
      try {
        final driver = await authRepository.verifyPhoneOtp(event.phone, event.otp, event.deviceId);
