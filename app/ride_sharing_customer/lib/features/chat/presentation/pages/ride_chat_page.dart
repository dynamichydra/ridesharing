...
class CustomerRideChatPage extends StatefulWidget {
  final String rideId;
  final String otherPartyName;
  final String? otherPartyAvatar;
  final String? otherPartyPhone;

  const CustomerRideChatPage({
    super.key,
  ...
  @override
  State<CustomerRideChatPage> createState() => _CustomerRideChatPageState();
}

class _CustomerRideChatPageState extends State<CustomerRideChatPage> {
  late final CustomerRideChatDataSource _chatDataSource;
  ...
  final List<String> _quickReplies = [
    ...
    'Please wait 2 minutes, coming down.',
  ...
  void initState() {
    super.initState();
    _chatDataSource = di.sl<CustomerRideChatDataSource>();
    _chatDataSource.ensureConnected(widget.rideId);
    _loadMessages();
    _listenIncoming();
  ...
  void _listenIncoming() {
    _chatSub = _chatDataSource.onChatMessage.listen((msg) {
      if ((msg.rideId.isEmpty || msg.rideId == widget.rideId) && mounted) {
          ...
          children: [
            CircleAvatar(
              ...
              child: widget.otherPartyAvatar == null || widget.otherPartyAvatar!.isEmpty
                  ? const Icon(Icons.drive_eta_rounded, color: Color(0xFF64748B), size: 22)
                  : null,
            ),
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              const Text(
                                'Direct messaging with your driver during the trip.',
                                ...
                                children: [
                                  if (!isMe) ...[
                                    CircleAvatar(
                                      radius: 13,
                                      backgroundColor: const Color(0xFFE2E8F0),
                                      child: const Icon(Icons.drive_eta_rounded, color: Color(0xFF64748B), size: 16),
                                    ),
